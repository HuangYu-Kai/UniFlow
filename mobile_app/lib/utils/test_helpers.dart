...
class PerformanceTestHelper {
  ...
  /// 測試數據加載時間
  static Future<Duration> testDataLoadingTime(
      Future<void> Function() loadData) async {
    final stopwatch = Stopwatch()..start();
    await loadData();
    stopwatch.stop();
...
class IntegrationTestScenarios {
  ...
  static Future<bool> testCompleteCareworkFlow() async {
    try {
      ...
      final membersValid =
          members.every(TestValidator.validateContributionScore);
  ...
  static Future<bool> testAiSuggestionGeneration() async {
    try {
      final healthData =
          TestDataGenerator.generateMockHealthData(isAbnormal: true);
